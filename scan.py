...
if __name__ == "__main__":
    parser = ArgParser(
        description="Scan from ADF, preprocess and upload do Docspell.",
    ...
    parser.add(
        "--docspell_url",
        required=True,
        help="Url to docspell, e.g. http://<docpsell_host>:7880/",
    )
    parser.add(
        "-n",
        "--name",
        type=str,
        default=uuid.uuid1(),
        help="Name of the scan. Default is a random String",
    )
    ...
    parser.add(
        ...
        "--manual_duplex",
        action="store_true",
        help="Scan front pages using ADF. Manual rotate pages. "
        "Scan back pages using ADF.",
    )
    parser.add(
        "-f",
        "--flatbed",
    ...
    parser.add(
        "--empty_threshold",
        type=float,
        help="Threshold to determine if a page is empty. "
        "The emptier a document is, the smaller it' value becomes.",
    )
    parser.add(
        "--device",
        help="Device name (e.g. SCANNER_ABC). "
        "Default: None (use systems default scanner)",
    )
    parser.add("--resolution", help="Scan resolution in dpi")
    parser.add(
        "--command_flatbed",
        help="scanimage option to perform a flatbed scan "
        "(it's depending on your scanner).",
    )
    parser.add(
        "--command_adf",
        help="scanimage option to perform a single-page adf scan "
        "(it's depending on your scanner).",
    )
    parser.add(
        "--command_duplex_adf",
        help="scanimage option to perform a duplex adf scan "
        "(it's depending on your scanner).",
    )
